...
public class PackageElement extends StatElement implements Comparable<PackageElement>, Serializable {
    ...
    public void add(StatElement element) {
        if (element instanceof WakelockStat) {
            m_duration += ((WakelockStat) element).getDuration();
            m_count += ((WakelockStat) element).getCount();
        } else if (element instanceof AlarmStat) {
            m_wakeups += ((AlarmStat) element).getWakeups();
