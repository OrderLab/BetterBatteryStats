 ...
 */
public class AlarmStat extends StatElement implements Comparable<AlarmStat>, Serializable {
    /**
     * the tag for logging
     */
    private static transient final String TAG = "AlarmStat";
     ...
     * @param strName
     */
    public AlarmStat(String strName) {
    ...
    public AlarmStat(String strName, String strDetails) {
        m_strPackageName = strName;
     ...
     * @param strName
     */
    public AlarmStat(String strName, String strDetails, long lWakeups, long lCount, long timeRunning, ArrayList<AlarmItem> items) {
        m_strPackageName = strName;
        m_details = strDetails;
        m_nWakeups = lWakeups;
    ...
    public AlarmStat(AlarmDto source) {
    ...
    public AlarmStat clone() {
        AlarmStat clone = new AlarmStat(m_strPackageName, m_details);
    ...
    /* (non-Javadoc)
     * @see edu.jhu.order.appstatstracker.batterystats.StatElement#getName()
     */
    ...
    public void substractFromRef(List<StatElement> myList) {
        if (myList != null) {
            for (int i = 0; i < myList.size(); i++) {
                try {
                    AlarmStat myRef = (AlarmStat) myList.get(i);
     ...
     */
    public int compareTo(AlarmStat o) {
    ...
    public static class CountComparator implements Comparator<AlarmStat> {
        public int compare(AlarmStat a, AlarmStat b) {
            return ((int) (b.getCount() - a.getCount()));
        }
    }

    public static class TimeComparator implements Comparator<AlarmStat> {
        public int compare(AlarmStat a, AlarmStat b) {
